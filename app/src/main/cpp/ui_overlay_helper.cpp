...
static void drawLine(float x1, float y1, float x2, float y2, float r, float g, float b) {
    // GL_LINES ile çizim — render thread'de çağrılır
...
static void drawHealthBar(float cx, float cy, float w, float h, float hp) {
    ...
    float r = hp < 30 ? 1.f : 0.f;
    float g = hp > 60 ? 1.f : hp/100.f;
...
void renderFrame() {
    ...
    for(int i = 0; i < g_playerCount; i++) {
        ...
        drawLine(g_screenW/2, g_screenH/2, cx, head.y, r, g, b);
    }
}
...
Java_com_renderkit_support_RenderSurface_nativeInit(JNIEnv* env, jobject, jobject surface) {
    g_initialized = true;
    LOGI("Native renderer init");
    return 1;
}
...
Java_com_renderkit_support_AssetHelper_setViewMatrix(JNIEnv*, jclass, jfloatArray mat) {
    ...
    // JNI ile float array okuma burada yapılır
    free(m);
}
